...
describe Todo::Service::Events::ListEvents do
  ...
  describe '#call' do
    it 'returns Success with empty list when no events' do
      result = Todo::Service::Events::ListEvents.new.call(requestor:, course_id: course.id)
    ...
    it 'returns Success with events when they exist' do
      location = Todo::Location.create(name: 'Room 101', course_id: course.id)
      Todo::Event.create(name: 'Event 1', course_id: course.id, location_id: location.id)
      Todo::Event.create(name: 'Event 2', course_id: course.id, location_id: location.id)
    ...
    it 'returns Failure for invalid course_id' do
      result = Todo::Service::Events::ListEvents.new.call(requestor:, course_id: 'invalid')
    ...
    it 'returns Failure for non-existent course' do
      result = Todo::Service::Events::ListEvents.new.call(requestor:, course_id: 999_999)
    ...
    it 'returns Failure when user has no access' do
      ...
      result = Todo::Service::Events::ListEvents.new.call(requestor: other_requestor, course_id: course.id)
