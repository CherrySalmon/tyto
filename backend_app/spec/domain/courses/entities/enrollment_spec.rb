...
describe 'Todo::Entity::Enrollment' do
  let(:now) { Time.now }
  ...
  describe 'creation' do
    it 'creates a valid enrollment' do
      enrollment = Todo::Entity::Enrollment.new(valid_attributes)
    ...
    it 'creates enrollment with multiple roles' do
      ...
      enrollment = Todo::Entity::Enrollment.new(attrs)
    ...
    it 'creates enrollment without optional attributes' do
      enrollment = Todo::Entity::Enrollment.new(
        id: nil,
        account_id: 10,
        course_id: 20,
        account_email: nil,
    ...
    it 'requires account_id' do
      _ { Todo::Entity::Enrollment.new(valid_attributes.merge(account_id: nil)) }
        .must_raise Dry::Struct::Error
    end

    it 'requires course_id' do
      _ { Todo::Entity::Enrollment.new(valid_attributes.merge(course_id: nil)) }
        .must_raise Dry::Struct::Error
    end

    it 'requires roles to be an array' do
      _ { Todo::Entity::Enrollment.new(valid_attributes.merge(roles: 'student')) }
        .must_raise Dry::Struct::Error
    end

    it 'rejects invalid role names' do
      _ { Todo::Entity::Enrollment.new(valid_attributes.merge(roles: ['invalid_role'])) }
        .must_raise Dry::Struct::Error
    end

    it 'rejects invalid email format' do
      _ { Todo::Entity::Enrollment.new(valid_attributes.merge(account_email: 'invalid-email')) }
        .must_raise Dry::Struct::Error
    end
  end
  ...
  describe '#has_role?' do
    it 'returns true when enrollment has the role' do
      enrollment = Todo::Entity::Enrollment.new(valid_attributes.merge(roles: %w[instructor student]))
    ...
    it 'returns false when enrollment lacks the role' do
      enrollment = Todo::Entity::Enrollment.new(valid_attributes.merge(roles: ['student']))
  ...
  describe 'role predicates' do
    it 'returns true for owner?' do
      enrollment = Todo::Entity::Enrollment.new(valid_attributes.merge(roles: ['owner']))
    ...
    it 'returns true for instructor?' do
      enrollment = Todo::Entity::Enrollment.new(valid_attributes.merge(roles: ['instructor']))
    ...
    it 'returns true for staff?' do
      enrollment = Todo::Entity::Enrollment.new(valid_attributes.merge(roles: ['staff']))
  ...
  describe '#teaching?' do
    ...
    it 'returns true when mixed roles include teaching role' do
      enrollment = Todo::Entity::Enrollment.new(valid_attributes.merge(roles: %w[student instructor]))
  ...
  describe '#active?' do
    ...
    it 'returns false when no roles' do
      enrollment = Todo::Entity::Enrollment.new(valid_attributes.merge(roles: []))
