...
describe 'Todo::Entity::Event' do
  ...
  describe 'creation' do
    it 'creates a valid event' do
      event = Todo::Entity::Event.new(valid_attributes)
    ...
    it 'rejects empty event name' do
      _ { Todo::Entity::Event.new(valid_attributes.merge(name: '')) }
        .must_raise Dry::Struct::Error
    end

    it 'rejects event name over 200 characters' do
      _ { Todo::Entity::Event.new(valid_attributes.merge(name: 'A' * 201)) }
        .must_raise Dry::Struct::Error
    end

    it 'requires course_id' do
      _ { Todo::Entity::Event.new(valid_attributes.merge(course_id: nil)) }
        .must_raise Dry::Struct::Error
    end

    it 'requires location_id' do
      _ { Todo::Entity::Event.new(valid_attributes.merge(location_id: nil)) }
        .must_raise Dry::Struct::Error
    end
  end
  ...
  describe '#time_range' do
    it 'returns TimeRange when start and end times exist' do
      ...
      _(event.time_range).must_be_instance_of Todo::Value::TimeRange
    ...
    it 'returns NullTimeRange when start_at is missing' do
      event = Todo::Entity::Event.new(valid_attributes.merge(start_at: nil))

      _(event.time_range).must_be_instance_of Todo::Value::NullTimeRange
      _(event.time_range.null?).must_equal true
    end

    it 'returns NullTimeRange when end_at is missing' do
      event = Todo::Entity::Event.new(valid_attributes.merge(end_at: nil))
