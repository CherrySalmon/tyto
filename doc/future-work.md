...
- [ ] **Forced logout of a specific account** — credentials are stateless (encrypted account id + expiry) and roles are read from the database per request, so a role change takes effect immediately without any session store. What that cannot do is end a particular session early, e.g. after a compromise. If wanted: add `sessions_valid_from` to `accounts`, put an issue time in the credential, and have `AuthToken::Mapper#from_auth_header` reject credentials issued before it; an admin action then bumps the timestamp. No Redis or session table needed. (Considered and deferred on `feat-new-members`, 2026-09-17.)
...
- [x] **Security tests** - Done on `feat-new-members`: `policies/account_spec.rb` and `routes/account_route_spec.rb` cover non-admins creating accounts, changing roles (own or others'), deleting, and reading account details, plus stale credentials after a demotion.
