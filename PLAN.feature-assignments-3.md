...
Last updated: 2026-04-26 (**Phase 1 green + post-review refactor + crypto consolidation**. Suite: 1292 runs / 0F / 0E / 1 skip / 98.33% coverage. **Phase 1 green:** new code under `infrastructure/file_storage/{mapper,limits,local_gateway,token_store,gateway,build_gateway,submission_mapper}.rb`; `aws-sdk-s3 ~>1.0` in Gemfile; `secrets_example.yml` documents new entries; `rake generate:local_storage_signing_key` task. **Post-review refactor:** sensitive-secret handling redesigned — per-module `setup(...)` cached on class state, single ENV-reading boundary at `config/initializers/credentials.rb` (with `ENV.delete` for every secret), `require_app.rb` gains a 3rd phase to load initializers after app code, `app/lib/env.rb` and `Tyto::Api.<accessor>` methods dropped, JWT_KEY migrated. **Crypto consolidation:** all RbNaCl calls collapsed behind `Tyto::Security` (`app/lib/security.rb`) with vendor-neutral names — `Tyto::Security::Secret` (encrypt/decrypt), `Tyto::Security::Signer` (sign/valid?), plus `random_bytes`, `unique_id`, `generate_secret_key`, `generate_signing_key`. `AuthToken::Gateway` and `FileStorage::TokenStore` now wrap those primitives; Rakefile, test helpers, and one spec switched off direct `RbNaCl::Random` calls. `require_app.rb` reordered so `lib/` loads before consumers. Convention documented in `doc/security.md` with a pointer from `.claude/CLAUDE.md`. Next action: Phase 2 — 3.8a red (IssueUploadUrls service spec).)
