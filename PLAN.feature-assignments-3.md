...
- [ ] 3.4 Add `aws-sdk-s3` gem + S3 config entries in `secrets_example.yml` and `config/storage.rb`. Create `infrastructure/file_storage/limits.rb` with `MAX_SIZE_BYTES` (R-P7). **Sensitive secret extraction (2026-04-26)**: in `backend_app/config/environment.rb`, extract `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `LOCAL_STORAGE_SIGNING_KEY` via `ENV.delete('VAR') or raise` at boot and expose via `Tyto::Api.<accessor>` (matching the centralization the Roda `:environments` plugin already provides for environment lookups). Slice 3 code (`build_gateway`, gateway constructors) reads from those accessors — never `ENV[...]` directly. **Existing `JWT_KEY` migration is a separate follow-up unless explicitly merged in here**. *(No gate — config + constant only.)*
...
Last updated: 2026-04-26 (3.1f red run complete: 3F new from `limits_spec.rb`, total 1270 runs / 1 skip / cumulative red 65 (15 failures + 50 errors) across 3.1a–3.1f. **Phase 1 red runs complete.** Plan note: env source-of-truth = `Tyto::Api.environment` (Roda `:environments` plugin); 3.4 will extract sensitive secrets via `ENV.delete + Tyto::Api.<accessor>`. Next action: Phase 1 green — 3.2 (Mapper implementation, blocked by 3.1a red).)
