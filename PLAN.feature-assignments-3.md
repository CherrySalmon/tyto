...
- [ ] 🚦 3.12 LocalGateway routes (`POST /_local_storage/upload` per R-P1, `GET /_local_storage/download/*key` splat per R-P5) mounted in the Roda route tree under `if Tyto::Api.environment.in?(%i[development test])` (allowlist per R-P3); single-use HMAC token validation with in-process nonce cache (R-P8); chrooted under `LOCAL_STORAGE_ROOT` — **BLOCKED by 3.8d red**. **green: ___P, total ___**
...
Last updated: 2026-04-26 (**Phase 2 — 3.8a red + 3.9 green + 3.8b red + 3.10 green + resource rename.** Suite: 1322 runs / 0F / 0E / 1 skip / 3036 assertions, deterministic. Route `POST /api/course/:course_id/assignments/:assignment_id/upload_grants` mounted in `course.rb`. **Resource rename (post-3.10):** `/upload_urls` → `/upload_grants`, `IssueUploadUrls` → `IssueUploadGrants`, and matching file renames. Reasoning: response is a 4-field credential `{requirement_id, key, upload_url, fields}`, so naming the collection after just the URL field undersells what's returned; "grant" borrows OAuth/IAM vocabulary for scoped, time-limited authorizations. Two side-quest fixes from 3.10: (a) service uses `created(...)` for proper 201; (b) `gateway_selection_spec.rb` was leaking `Tyto::FileStorage` class state across files — added `snapshot_file_storage`/`restore_file_storage` helpers via `instance_variable_get`/`set` and `before/after` hooks per describe. New `.rubocop.yml` at repo root relaxes `Layout/HashAlignment` to accept compact + tabbed styles per-hash. Next action: 3.8c red (CreateSubmission updates — server-reconstructed key + HEAD-check + extension-change cleanup + content_type-as-untrusted test).)
